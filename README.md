...
The Residual mean deviance(tree1):  0.4413 = 124.9 / 283 

Misclassification error rate(tree1): 0.1171 (11.71%) = 35 / 299

terminal nodes = 8

If we use only use vitals of the patient to plot classification tree2 and exclude the 'time' variable which is the follow-up period in days (as shown above), we observe that 'serum_creatinine' was the most important variable(variable importance = 46), followed by 'ejection_fraction' and 'age' which is consistent for both arguments tree() as well as rpart(). The residual mean deviance now increased to 0.854 and misclassification error rate also increased to 0.1906 with 9 terminal nodes.

Residual mean deviance(tree2):  0.854 = 247.7 / 290 
...
Plotting the survived versus dead patients data with ejection_fraction on x-axis and follow up time on y axis as these were the top most important variavles according to the decision tree model with lowest clasiificiation error rate.

```{r, echo = FALSE,out.width="50%", fig.align='center'}
# Plot tree for plot only demonstration
...
Before using the newpatients data for prediction, coersed the categorical variables into factors as done above for heart data.
...
Conclusion: On the basis of the prediction based on classification tree, there were 13 new patients in the dataset that are under high risk for mortality with probability for 'Dead' greater than 75% (as shown in table above).
